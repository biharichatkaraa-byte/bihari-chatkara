...
const POS: React.FC<POSProps> = ({ orders, menuItems, onPlaceOrder, onUpdateOrder, onUpdatePayment, onUpdateMenuItem, currentUserName, userRole }) => {
  ...
  const handleItemClick = (item: MenuItem) => {
    ...
    const hasPortions = item.portionPrices && (
        (item.portionPrices.half && item.portionPrices.half > 0) || 
        (item.portionPrices.quarter && item.portionPrices.quarter > 0)
    );
  ...
  const confirmPortionSelection = () => {
      ...
      const price = getPortionPrice(portionItem, selectedPortionType);
      addToCart(portionItem, selectedPortionType, price, portionQuantity);
      setPortionItem(null);
  };
  ...
  const handleAddCustomItem = () => {
      ...
      const newItem: LineItem = {
          id: `l-custom-${Date.now()}`,
          menuItemId: 'custom-modal',
          name: customItemName,
          quantity: qty,
          priceAtOrder: price,
          portion: customItemPortion,
          modifiers: ['Custom Item']
      };
  ...
  const handleAddItemNote = (itemId: string) => {
      const note = prompt("Enter note for this item (e.g., 'No Spicy', 'Extra Sauce'):");
      if (note !== null) {
          setCurrentCart(currentCart.map(item => 
              item.id === itemId ? { ...item, modifiers: note ? [note] : [] } : item
          ));
      }
  };
      ...
      <div className={`flex-1 flex flex-col min-w-0 h-full ${showMobileCart ? 'hidden lg:flex' : 'flex'}`}>
        <div className="flex flex-wrap items-center gap-2 mb-4">
             ...
             <div className="relative flex-1 min-w-[200px]">
                 ...
                 <input type="text" placeholder="Search menu items..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 shadow-sm" />
                 {searchQuery && <button onClick={() => setSearchQuery('')} className="absolute right-3 top-2.5 text-slate-400 hover:text-slate-600"><X size={18} /></button>}
             </div>
             <button onClick={() => setShowSoldOut(!showSoldOut)} className={`px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 border transition-colors ${showSoldOut ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-500 border-slate-300 hover:bg-slate-50'}`}>
                 {showSoldOut ? <Eye size={16} /> : <EyeOff size={16} />} <span className="hidden xl:inline">Sold Out</span>
             </button>
             <button onClick={() => setShowCustomItemModal(true)} className="bg-slate-800 text-white px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-slate-700 whitespace-nowrap"><Zap size={16} className="text-yellow-400" /> Custom</button>
        </div>

        <div className="flex gap-2 flex-wrap mb-2 pb-1">
            {categories.map(cat => (
                <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-lg whitespace-nowrap font-medium transition-colors text-sm ${selectedCategory === cat ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'}`}>
                {cat}
                </button>
            ))}
        </div>
        ...
        {subCategories.length > 0 && (
            <div className="flex gap-2 flex-wrap mb-4 pb-1 animate-in fade-in slide-in-from-top-1">
                <button onClick={() => setSelectedSubCategory('All')} className={`px-3 py-1.5 rounded-full whitespace-nowrap text-xs font-bold transition-colors border ${selectedSubCategory === 'All' ? 'bg-slate-700 text-white border-slate-700' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-200'}`}>All {selectedCategory}</button>
                {subCategories.map(sub => (
                    <button key={sub} onClick={() => setSelectedSubCategory(sub)} className={`px-3 py-1.5 rounded-full whitespace-nowrap text-xs font-bold transition-colors border ${selectedSubCategory === sub ? 'bg-slate-700 text-white border-slate-700' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>{sub}</button>
                ))}
            </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 lg:gap-4 overflow-y-auto pr-1 pb-20 lg:pb-4 flex-1 content-start">
          {filteredItems.map(item => {
            ...
            return (
            <div key={item.id} onClick={() => handleItemClick(item)} className={`bg-white p-3 lg:p-4 rounded-xl shadow-sm border transition-all text-left flex flex-col min-h-[140px] h-full justify-between group relative overflow-hidden ${isOutOfStock ? 'border-slate-200 opacity-70 cursor-not-allowed bg-slate-50' : 'border-slate-200 hover:shadow-md hover:border-blue-300 cursor-pointer'}`}>
              <div>
                <h3 className={`font-bold text-sm lg:text-base text-slate-800 leading-tight flex flex-wrap items-center gap-2 ${!isOutOfStock && 'group-hover:text-blue-600'}`}>
                    {item.isVeg !== undefined && <span className={`min-w-3 w-3 h-3 rounded-full border ${item.isVeg ? 'border-green-600 bg-green-500' : 'border-red-600 bg-red-500'}`}></span>}
                    <span className="break-words w-full">{item.name}</span>
                </h3>
                <p className="text-xs text-slate-500 mt-1">{item.subCategory ? <span className="font-semibold text-slate-400">{item.subCategory} • </span> : ''}{item.description}</p>
              </div>
              <div className="flex justify-between items-center mt-2 relative z-10">
                <span className={`font-bold ${isOutOfStock ? 'text-slate-400' : 'text-slate-900'}`}>₹{item.price.toFixed(0)}</span>
                {isOutOfStock ? (
                    <span className="text-[10px] font-bold bg-red-100 text-red-600 px-2 py-1 rounded-md flex items-center gap-1"><Ban size={10} /> SOLD OUT</span>
                ) : (
                    <button onClick={(e) => { e.stopPropagation(); handleItemClick(item); }} className="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-colors" title="Add to Order"><Plus size={16} /></button>
                )}
              </div>
              {userRole === UserRole.MANAGER && (
                  <button onClick={(e) => handleToggleAvailability(item, e)} className={`absolute top-2 right-2 p-1.5 rounded-md transition-colors z-20 ${isOutOfStock ? 'bg-red-50 text-red-500 hover:bg-red-100' : 'bg-slate-50 text-slate-400 hover:bg-slate-200 hover:text-slate-600'}`} title={isOutOfStock ? "Mark as Available" : "Mark as Sold Out"}><Power size={14} /></button>
              )}
              <div className="absolute inset-0 bg-transparent group-active:bg-slate-100/50 pointer-events-none" />
            </div>
          )})}
          {filteredItems.length === 0 && <div className="col-span-full flex flex-col items-center justify-center py-12 text-slate-400"><p>No items found.</p>{!showSoldOut && <p className="text-xs mt-1">Sold out items are hidden. Use the eye icon to view them.</p>}</div>}
        </div>
      </div>

      {!showMobileCart && currentCart.length > 0 && (
          <div className="lg:hidden fixed bottom-6 left-4 right-4 z-40">
              <button onClick={() => setShowMobileCart(true)} className="w-full bg-slate-900 text-white p-4 rounded-xl shadow-2xl flex justify-between items-center animate-in slide-in-from-bottom-5">
                  <div className="flex items-center gap-3">
                      <div className="bg-orange-500 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">{currentCart.reduce((acc, i) => acc + i.quantity, 0)}</div>
                      <span className="font-medium text-sm">Items Added</span>
                  </div>
                  <div className="flex items-center gap-2"><span className="font-bold text-lg">₹{calculateSubtotal().toFixed(0)}</span><ChevronUp size={20} /></div>
              </button>
          </div>
      )}

      <div className={`lg:w-96 bg-white flex flex-col border border-slate-200 shadow-xl lg:rounded-2xl ${showMobileCart ? 'fixed inset-0 z-50 w-full rounded-none animate-in slide-in-from-bottom-10' : 'hidden lg:flex h-full rounded-2xl'}`}>
        {showMobileCart && (
            <div className="p-4 bg-slate-900 text-white flex items-center justify-between lg:hidden">
                <button onClick={() => setShowMobileCart(false)} className="flex items-center gap-2 text-slate-300 hover:text-white"><ChevronDown size={20} /> Back to Menu</button>
        ...
        <div className="p-5 border-b border-slate-100 hidden lg:block">
          <div className="flex justify-between items-center">
              <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2"><ShoppingCart size={20} /> {editingOrder ? `Order #${editingOrder.id.split('-')[1]}` : 'Current Order'}</h2>
              {currentCart.length > 0 && <button onClick={handleClearCart} className="text-slate-400 hover:text-red-500 transition-colors" title="Clear Cart"><Trash2 size={18} /></button>}
          </div>
          <div className="flex justify-between items-center mt-2">
             <p className="text-sm font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded">{selectedTable ? `Table #${selectedTable}` : 'Takeaway / Quick Order'}</p>
             {selectedTable && <button onClick={() => { setSelectedTable(null); setEditingOrder(null); setActiveView('tables'); }} className="text-xs text-slate-400 underline hover:text-red-500">Change Table</button>}
          </div>
        </div>

        <div className="p-3 bg-slate-50 border-b border-slate-200 animate-in fade-in">
             <div className="flex gap-2">
                 <input type="text" className="flex-1 min-w-0 px-2 py-1.5 text-xs border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Custom Item Name" value={cartCustomName} onChange={e => setCartCustomName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleCartCustomAdd()} />
                 <div className="relative w-20">
                     <span className="absolute left-2 top-1.5 text-xs text-slate-400">₹</span>
                     <input type="number" className="w-full pl-5 pr-2 py-1.5 text-xs border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Price" value={cartCustomPrice} onChange={e => setCartCustomPrice(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleCartCustomAdd()} />
                 </div>
                 <button onClick={handleCartCustomAdd} disabled={!cartCustomName || !cartCustomPrice} className="px-3 py-1.5 bg-slate-800 text-white rounded text-xs font-bold hover:bg-slate-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-1"><Plus size={12} /> Add</button>
             </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50">
          {currentCart.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12">
              <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-4 text-slate-300"><ShoppingCart size={32} /></div>
              ...
              return (
                <div key={item.id} className="p-3 bg-white border border-slate-200 rounded-xl shadow-sm relative group transition-all hover:border-blue-300 hover:shadow-md">
                  ...
                  <div className="flex justify-between items-center mt-3 pt-2 border-t border-slate-100">
                      <button onClick={() => handleAddItemNote(item.id)} className="text-[10px] font-bold text-slate-400 hover:text-blue-600 flex items-center gap-1 uppercase tracking-wide transition-colors"><MessageSquare size={12} /> {item.modifiers?.length ? 'Edit Note' : 'Add Note'}</button>
                      <div className="flex items-center gap-3 bg-slate-100 rounded-lg p-1">
                        <button onClick={() => updateQuantity(item.id, -1)} className="w-6 h-6 flex items-center justify-center bg-white rounded text-slate-600 shadow-sm hover:text-red-500 transition-colors">{item.quantity === 1 ? <Trash2 size={14} /> : <Minus size={14} />}</button>
                        <span className="font-bold text-slate-800 text-sm w-4 text-center">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, 1)} className="w-6 h-6 flex items-center justify-center bg-white rounded text-slate-600 shadow-sm hover:text-blue-600 transition-colors"><Plus size={14} /></button>
                      </div>
                  </div>
                </div>
              )
        ...
        <div className="p-6 bg-white border-t border-slate-200 rounded-b-none lg:rounded-b-2xl space-y-3 pb-safe-area shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10">
          <div className="grid grid-cols-2 gap-3 mb-2">
              <div>
                  ...
                  <input type="number" value={manualTaxRate} onChange={e => setManualTaxRate(e.target.value)} placeholder="0" className="w-full px-2 py-1.5 text-xs font-bold border border-slate-200 rounded bg-slate-50 focus:ring-1 focus:ring-blue-500 focus:bg-white transition-colors" />
              </div>
              <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Discount (₹)</label>
                  <input type="number" value={manualDiscount} onChange={e => setManualDiscount(e.target.value)} placeholder="0" className="w-full px-2 py-1.5 text-xs font-bold border border-slate-200 rounded bg-slate-50 focus:ring-1 focus:ring-blue-500 focus:bg-white transition-colors" />
              </div>
          </div>

          <div className="flex justify-between text-slate-500 text-xs"><span>Subtotal</span><span>₹{subtotal.toFixed(2)}</span></div>
          {discount > 0 && <div className="flex justify-between text-green-600 text-xs font-bold"><span>Discount</span><span>-₹{discount.toFixed(2)}</span></div>}
          <div className="flex justify-between text-slate-500 text-xs"><span>GST ({taxRate}%)</span><span>₹{taxAmount.toFixed(2)}</span></div>
          ...
          {!showPaymentOptions ? (
              <div className="flex flex-col gap-2 pt-2">
                <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => { setShowPaymentOptions(true); setPaymentStep('method'); }} disabled={currentCart.length === 0} className="flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-green-500 text-green-700 bg-green-50 font-bold hover:bg-green-100 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95"><CreditCard size={18} /> Pay Now</button>
                    <button onClick={handleKitchenSend} disabled={currentCart.length === 0} className={`flex items-center justify-center gap-2 py-3 rounded-xl text-white font-bold shadow-lg transition-all active:scale-95 ${orderSentSuccess ? 'bg-green-600' : 'bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-blue-200'}`}>{orderSentSuccess ? <Check size={18} /> : <Send size={18} />}{orderSentSuccess ? 'Sent!' : (editingOrder ? 'Update Order' : 'Send to KDS')}</button>
                </div>
                <div className="grid grid-cols-2 gap-2">
                    <button onClick={handleSkipBill} disabled={currentCart.length === 0} className="flex items-center justify-center gap-2 py-2 rounded-lg bg-slate-100 text-slate-600 text-xs font-bold hover:bg-slate-200 disabled:opacity-50 transition-colors"><SkipForward size={14} /> Save Draft</button>
                    <button onClick={handlePrintOnly} disabled={currentCart.length === 0} className="flex items-center justify-center gap-2 py-2 rounded-lg bg-slate-100 text-slate-600 text-xs font-bold hover:bg-slate-200 disabled:opacity-50 transition-colors"><Printer size={14} /> Print Bill</button>
                </div>
            </div>
          ) : (
             <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 bg-slate-50 p-3 rounded-xl border border-slate-200 relative">
                 <div className="flex justify-between items-center mb-1"><p className="text-xs font-bold text-slate-500 uppercase">{paymentStep === 'cash' ? 'Cash Settlement' : 'Select Payment Method'}</p><button onClick={() => setShowPaymentOptions(false)} className="text-slate-400 hover:text-slate-600"><X size={16}/></button></div>
                 {paymentStep === 'method' ? (
                     <div className="grid grid-cols-2 gap-3">
                        <button onClick={handleInitiateCashPayment} className="flex flex-col items-center justify-center p-4 rounded-xl bg-white border-2 border-green-100 text-green-700 hover:border-green-500 hover:shadow-md transition-all active:scale-95"><Banknote size={24} className="mb-2" /><span className="text-xs font-bold">Cash</span></button>
                        <button onClick={() => handlePayment(PaymentMethod.ONLINE)} className="flex flex-col items-center justify-center p-4 rounded-xl bg-white border-2 border-purple-100 text-purple-700 hover:border-purple-500 hover:shadow-md transition-all active:scale-95"><Smartphone size={24} className="mb-2" /><span className="text-xs font-bold">UPI / Online</span></button>
                        <button onClick={handlePaytmPayment} className="col-span-2 flex flex-col items-center justify-center p-4 rounded-xl bg-white border-2 border-blue-900 text-blue-900 hover:bg-blue-50 transition-all active:scale-95"><TabletSmartphone size={24} className="mb-2" /><span className="text-xs font-bold">Paytm Terminal (POS)</span></button>
                     </div>
                 ) : (
                     <div className="space-y-3 animate-in slide-in-from-right-2">
                         <div className="bg-white p-3 rounded-lg border border-slate-200 text-center"><span className="text-xs text-slate-400 uppercase">Amount Due</span><div className="text-2xl font-bold text-slate-800">₹{total.toFixed(0)}</div></div>
                         <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Cash Tendered</label><div className="relative"><span className="absolute left-3 top-2.5 text-slate-400 font-bold">₹</span><input type="number" value={cashTendered} onChange={e => setCashTendered(e.target.value)} className="w-full pl-8 pr-3 py-2 text-lg font-bold border border-slate-300 rounded-lg focus:ring-2 focus:ring-green-500" placeholder="0" autoFocus /></div></div>
                         <div className="flex gap-2">{[100, 200, 500, 2000].map(amt => (<button key={amt} onClick={() => setCashTendered(amt.toString())} className="flex-1 py-1.5 bg-slate-200 text-slate-600 rounded text-xs font-bold hover:bg-slate-300">₹{amt}</button>))} <button onClick={() => setCashTendered(total.toFixed(0))} className="flex-1 py-1.5 bg-blue-100 text-blue-700 rounded text-xs font-bold hover:bg-blue-200">Exact</button></div>
                         <div className="flex justify-between items-center bg-green-50 p-3 rounded-lg border border-green-100"><span className="text-xs font-bold text-green-700 uppercase">Change Due</span><span className="text-xl font-black text-green-700">₹{change.toFixed(0)}</span></div>
                         <div className="flex gap-2 pt-1"><button onClick={() => setPaymentStep('method')} className="px-3 py-3 rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 font-bold text-xs">Back</button><button onClick={() => handlePayment(PaymentMethod.CASH)} className="flex-1 py-3 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 shadow-lg active:scale-95 flex items-center justify-center gap-2"><Check size={18} /> Complete Payment</button></div>
                     </div>
                 )}
                 {isPaytmProcessing && <div className="absolute inset-0 bg-white/90 backdrop-blur-sm z-20 flex flex-col items-center justify-center text-center rounded-xl p-4 animate-in fade-in"><div className="w-16 h-16 bg-blue-900 rounded-full flex items-center justify-center mb-4 shadow-xl"><Loader2 size={32} className="text-white animate-spin" /></div><h4 className="font-bold text-slate-800 text-lg mb-1">Sending to Terminal...</h4><p className="text-xs text-slate-500">Please complete payment on the machine.</p></div>}
             </div>
          )}
        </div>
      </div>
    ...
    <div className="h-full w-full relative">
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-white rounded-2xl shadow-2xl p-6 w-96 transform transition-all scale-100">
                  <div className="flex justify-between items-start mb-6">
                      <div><h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">{portionItem.isVeg !== undefined && <span className={`w-3 h-3 rounded-full border ${portionItem.isVeg ? 'border-green-600 bg-green-500' : 'border-red-600 bg-red-500'}`}></span>}{portionItem.name}</h3><p className="text-slate-500 text-sm">Select size & quantity</p></div>
                      <button onClick={() => setPortionItem(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
                  </div>
                  <div className="space-y-6">
                      <div><label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Portion Size</label><div className="flex p-1 bg-slate-100 rounded-xl">{(['Full', 'Half', 'Quarter'] as PortionType[]).filter(type => getPortionPrice(portionItem, type) > 0).map((type) => { const price = getPortionPrice(portionItem, type); const isSelected = selectedPortionType === type; return (<button key={type} onClick={() => setSelectedPortionType(type)} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${isSelected ? 'bg-white text-blue-600 shadow-sm ring-1 ring-black/5' : 'text-slate-500 hover:text-slate-700'}`}><div className="flex flex-col items-center"><span>{type}</span><span className="text-[10px] font-normal opacity-80">₹{price.toFixed(0)}</span></div></button>)})}</div></div>
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100"><label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Quantity</label><div className="flex items-center justify-center gap-4"><button onClick={() => setPortionQuantity(Math.max(1, portionQuantity - 1))} className="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-100"><Minus size={18} /></button><span className="text-2xl font-bold text-slate-800 w-12 text-center">{portionQuantity}</span><button onClick={() => setPortionQuantity(portionQuantity + 1)} className="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-100"><Plus size={18} /></button></div></div>
                      <button onClick={confirmPortionSelection} className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200">Add to Order - ₹{(getPortionPrice(portionItem, selectedPortionType) * portionQuantity).toFixed(0)}</button>
                  </div>
              </div>
          </div>
       )}
       ...
       {showCustomItemModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-white rounded-2xl shadow-2xl p-6 w-80 transform transition-all scale-100">
                   <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Zap size={20} className="text-yellow-500" /> Quick Custom Item</h3><button onClick={() => setShowCustomItemModal(false)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button></div>
                   <div className="space-y-4">
                       <div><label className="block text-xs font-bold text-slate-500 mb-1">Item Name</label><input type="text" value={customItemName} onChange={e => setCustomItemName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddCustomItem()} className="w-full border rounded px-3 py-2" placeholder="e.g. Open Food" /></div>
                       <div><label className="block text-xs font-bold text-slate-500 mb-2">Select Portion (Label Only)</label><div className="flex gap-2">{(['Full', 'Half', 'Quarter'] as PortionType[]).map(p => (<button key={p} onClick={() => setCustomItemPortion(p)} className={`flex-1 py-2 text-xs font-bold rounded-lg border-2 transition-all ${customItemPortion === p ? 'bg-slate-800 text-white border-slate-800 shadow-md' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}>{p}</button>))}</div></div>
                       <div className="grid grid-cols-2 gap-2"><div><label className="block text-xs font-bold text-slate-500 mb-1">Price (₹)</label><div className="relative"><span className="absolute left-2 top-2 text-slate-400 text-xs">₹</span><input type="number" value={customItemPrice} onChange={e => setCustomItemPrice(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddCustomItem()} className="w-full border rounded pl-5 pr-2 py-2 font-bold" placeholder="0" /></div></div><div><label className="block text-xs font-bold text-slate-500 mb-1">Quantity</label><input type="number" value={customItemQty} onChange={e => setCustomItemQty(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddCustomItem()} className="w-full border rounded px-3 py-2 font-bold text-center" placeholder="1" /></div></div>
                       <button onClick={handleAddCustomItem} disabled={!customItemName || !customItemPrice} className="w-full py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 mt-2 shadow-lg disabled:opacity-50">Add Item to Cart</button>
                   </div>
              </div>
          </div>
       )}
       ...
       {showPrintModal && printOrder && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
              <div id="printable-receipt" className="bg-white p-4 rounded-none w-[320px] shadow-2xl flex flex-col text-black relative font-mono text-sm print:w-full print:shadow-none print:absolute print:inset-0">
                  <button onClick={() => setShowPrintModal(false)} className="absolute top-2 right-2 text-slate-400 hover:text-slate-600 print-hidden bg-slate-100 p-1 rounded-full z-10"><X size={16}/></button>
                      ...
                      <div className="relative group">
                          {isEditingReceipt ? (
                              ...
                              <>
                                  ...
                                  <button onClick={() => setIsEditingReceipt(true)} className="absolute top-0 right-0 p-1 text-slate-300 hover:text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity print-hidden"><Pencil size={12} /></button>
                              </>
                          )}
                      </div>
                      <div className="mt-2 text-[10px] flex justify-between border-t border-dashed border-black pt-1"><span>Date: {format(new Date(), 'dd/MM/yyyy')}</span><span>Time: {format(new Date(), 'HH:mm')}</span></div>
                      <div className="text-[10px] flex justify-between"><span>Bill No: {printOrder.id.split('-')[1] || printOrder.id}</span><span>T.No: {printOrder.tableNumber}</span></div>
                      <div className="text-[10px] text-left">Cashier: {currentUserName}</div>
                  </div>

                  <div className="space-y-1 mb-2 text-xs">
                      <div className="flex font-bold border-b border-black pb-1 mb-1"><span className="flex-1">Item</span><span className="w-8 text-center">Qty</span><span className="w-12 text-right">Amt</span></div>
                      {printOrder.items.map((item, idx) => {
                           ...
                           return (
                               <div key={idx} className="flex justify-between items-start">
                                  ...
                                  <span className="w-12 text-right">{(price * item.quantity).toFixed(0)}</span>
                               </div>
                           )
                      })}
                  </div>
                  ...
                  <div className="border-t-2 border-dashed border-black pt-2 space-y-1 text-xs">
                      ...
                      <div className="flex justify-between font-bold text-lg mt-2 pt-1 border-t border-black"><span>TOTAL</span><span>{getOrderTotal(printOrder).toFixed(2)}</span></div>
                      <div className="flex justify-between text-[10px] mt-1"><span>Pay Mode:</span><span className="uppercase">{printOrder.paymentMethod === PaymentMethod.PAYTM_POS ? 'PAYTM MACHINE' : (printOrder.paymentMethod || 'DRAFT')}</span></div>
                  </div>

                  <div className="text-center mt-4 mb-2 text-[10px]"><p className="font-bold">*** Thank You Visit Again ***</p><p>For Feedback: {receiptDetails.phone}</p></div>
                  <div className="space-y-2 print-hidden mt-2"><button onClick={() => window.print()} className="w-full py-2 bg-black text-white font-bold rounded hover:bg-slate-800 flex items-center justify-center gap-2"><Printer size={16} /> Print Receipt</button></div>
              </div>
          </div>
       )}
    </div>
